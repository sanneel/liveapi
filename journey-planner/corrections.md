...
- Never assert that an activity is uncaptured from memory — read the RECIPES CATALOG and the composable-types list in this prompt. (The correction that used to live here patched a stale "uncaptured" claim in the KB and backlog; both are fixed at source now, so there is nothing to override. A correction whose only job is to contradict another section of the same prompt is worse than no correction: the model sees both.)
...
- A spec must preserve blockers (⛔ survives into the machine spec):
...
- A COMMS JOURNEY IS NEVER A BARE CHAIN OF SENDS (HARD RULE — this is the #1 cause of useless comms plans):
  ...
  - `wait_date` is NOT composable — `chain_types` has only `wait_interval` (alias `wait`, ISO-8601 via the `wait` setting). A brief that needs an absolute gate date is either a `graph` spec against a reference that already has `wait_date`, or ⛔ RESOLVE_AT_BUILD_TIME. Never emit `wait_date` in a chain — it is rejected as an unknown type.
