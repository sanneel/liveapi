...
app = FastAPI(title="Fights Hot PNG Renderer", version="1.0")
...
def _display_fighter_name(ev: Dict[str, Any], side: str) -> str:
...
def _normalize_live_time(tr: str) -> str:
...
def _normalize_prematch_time(tr: str) -> str:
    s = " ".join((tr or "").strip().split())
    if not s:
        return ""
...
def _header_center_text(ev: Dict[str, Any]) -> str:
    ...
    if status == "live":
        return _normalize_live_time(tr)

    return _normalize_prematch_time(tr)
...
# ---------- Fighter font autosize ----------
def _fit_fighter_font_size(
    draw: ImageDraw.ImageDraw,
    home_name: str,
    away_name: str,
    max_home_w: int,
...
def render_hot_png(events: List[Dict[str, Any]]) -> bytes:
    ...
    for ev in events[:n]:
        ...
        home_name = _display_fighter_name(ev, "home")
        away_name = _display_fighter_name(ev, "away")
        ...
        fitted_size = _fit_fighter_font_size(
            d,
            home_name,
            away_name,
            max_home_w,
            max_away_w,
            min_size=MIN_FIGHTER_FONT,
        )
        font_fighter = _pick_font(fitted_size)

        d.text((left_name_x, baseline_y + FIGHTER_Y_OFFSET), home_name, fill=text_main, font=font_fighter)

        away_w = d.textlength(away_name, font=font_fighter)
        d.text((right_name_right_edge - away_w, baseline_y + FIGHTER_Y_OFFSET), away_name, fill=text_main, font=font_fighter)
...
def fetch_hot_json(limit: int) -> Dict[str, Any]:
    url = f"{DATA_API_BASE}/events/fights/hot"
...
# ---------- API ----------
@app.get("/render/fights/hot.png")
def render_fights_hot_png(limit: int = DEFAULT_LIMIT) -> Response:
...
@app.get("/render/fights/manual/{slot}.png")
def render_fights_manual_png(slot: str) -> Response:
...
def health() -> Dict[str, Any]:
    ...
    return {
        ...
        "fighter_font_base": BASE_FIGHTER_FONT,
        "fighter_font_min": MIN_FIGHTER_FONT,
        "live_score_font": LIVE_SCORE_FONT,
        "endpoint": "/render/fights/hot.png",
    }