...
check("two channels, no splits or waits asked for",
      [n["type"] for n in spec["chain"]] == ["nc", "sms"],
      str([n["type"] for n in spec["chain"]]))
# ...but that shape does not BUILD: a delivered message never leads straight into
# another send (0 occurrences in 18 captures), so the composer refuses it. The
# builder passes the picks through faithfully and lets that refusal speak.
import journey_composer as _JC  # noqa: E402
_errs = _JC.verify(_JC.compose(spec)["body"])
check("and the composer refuses it, naming the fix",
      any("never straight to another send" in e for e in _errs), str(_errs[:1]))

spec, _ = build(channels=["nc", "sms"], waits={"nc": "2h"})
check("a wait between them makes it buildable",
      not _JC.verify(_JC.compose(spec)["body"]),
      str(_JC.verify(_JC.compose(spec)["body"])[:1]))
...
check("an 'Offer Link' row is read as the link",
      q.raw_link.endswith("raspa-nov"), repr(q.raw_link))
